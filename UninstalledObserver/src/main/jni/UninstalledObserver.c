...
static const char APP_DIR[] = "/data/data/com.carlisle.songtaste";
static const char APP_FILES_DIR[] = "/data/data/com.carlisle.songtaste/files";
static const char APP_OBSERVED_FILE[] = "/data/data/com.carlisle.songtaste/files/observedFile";
static const char APP_LOCK_FILE[] = "/data/data/com.carlisle.songtaste/files/lockFile";
/* 内全局变量 */

/* Class:     com.carlisle.songtaste
 * Method:    init
 * Signature: ()V
 * return: 子进程pid
 */
JNIEXPORT int JNICALL Java_com_carlisle_songtaste_base_UninstalledObserver_init(JNIEnv *env, jobject obj, jstring userSerial, jstring website)
{
    ...
    {
	    LOG_DEBUG((*env)->GetStringUTFChars(env, tag, &isCopy), (*env)->GetStringUTFChars(env, (*env)->NewStringUTF(env, "fork Success !!!"), &isCopy));
        // 若监听文件所在文件夹不存在，创建
        ...
        if (p_filesDir == NULL)
        {
            ...
            LOG_DEBUG((*env)->GetStringUTFChars(env, tag, &isCopy), (*env)->GetStringUTFChars(env, (*env)->NewStringUTF(env, "mkdir Success !!!"), &isCopy));
        }
        LOG_DEBUG((*env)->GetStringUTFChars(env, tag, &isCopy), (*env)->GetStringUTFChars(env, (*env)->NewStringUTF(env, "app dir is exist !!!"), &isCopy));
        
        // 若被监听文件不存在，创建文件
        FILE *p_observedFile = fopen(APP_OBSERVED_FILE, "r");
        if (p_observedFile == NULL)
        {
            LOG_DEBUG((*env)->GetStringUTFChars(env, tag, &isCopy), (*env)->GetStringUTFChars(env, (*env)->NewStringUTF(env, "the observed file is not existed!!!"), &isCopy));
            p_observedFile = fopen(APP_OBSERVED_FILE, "w");

            if (p_observedFile == NULL) {
                LOG_DEBUG((*env)->GetStringUTFChars(env, tag, &isCopy), (*env)->GetStringUTFChars(env, (*env)->NewStringUTF(env, "the observed file create falied!!!"), &isCopy));
            }
        ...
        while(1)
        {
            ...
            {
            	LOG_DEBUG((*env)->GetStringUTFChars(env, tag, &isCopy), (*env)->GetStringUTFChars(env, (*env)->NewStringUTF(env, "the observer file is deleted"), &isCopy));
