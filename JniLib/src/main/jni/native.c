...
jstring
Java_com_carlisle_songtaste_ui_main_SplashActivity_init( JNIEnv* env,
                                                 jobject thiz )
{
    jstring tag = (*env)->NewStringUTF(env, c_TAG);
    ...
    else if (pid == 0)
    {
        ...
        int watchDescriptor;
        watchDescriptor = inotify_add_watch(fileDescriptor, "/data/data/com.carlisle.songtaste", IN_DELETE);
